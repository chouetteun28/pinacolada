...
class PoseEstimator:
    def __init__(self, pose=True, face_mesh=True, face_detection=True, bounding_box=True):
        ...
        if self.ifpose:
            ...
            self.pose = self.mp_pose.Pose(
                static_image_mode=False, min_detection_confidence=0.5, min_tracking_confidence=0.5, model_complexity=2)
        if self.iffacemesh:
            self.mp_face_mesh = mp.solutions.face_mesh
            self.face_mesh = self.mp_face_mesh.FaceMesh(
                max_num_faces=5, refine_landmarks=True, min_detection_confidence=0.3, min_tracking_confidence=0.5)
        if self.iffacedetection:
            self.mp_face_detection = mp.solutions.face_detection
            self.face_detection = self.mp_face_detection.FaceDetection(
                model_selection=1, min_detection_confidence=0.2)
        if self.ifboundingbox:
            self.boundingbox_model = torch.hub.load(
                'ultralytics/yolov5', 'yolov5s')

    def get_pose(self, frame: np.ndarray):
        return self.pose.process(frame)

    def get_face_mesh(self, frame: np.ndarray):
        return self.face_mesh.process(frame)

    def get_face_detection(self, frame: np.ndarray):
        return self.face_detection.process(frame)

    def get_boundingbox(self, frame: np.ndarray):
        results = self.boundingbox_model(frame)
        df = results.pandas().xyxy[0]
        return df

    def draw_pose(self, frame: np.ndarray, results):
    ...
    def draw_face_mesh(self, frame: np.ndarray, results):
    ...
    def draw_boundingbox(self, frame, results):
        for index, row in results.iterrows():
            if row['confidence'] > 0.5 and row['name'] == 'person':
                cv2.rectangle(frame, (int(row['xmin']), int(row['ymin'])), (int(
                    row['xmax']), int(row['ymax'])), (0, 255, 0), 2)

    def process(self, frame: np.ndarray, draw_pose: bool = True, draw_face_mesh: bool = True, draw_face_detection: bool = True, draw_boundingbox: bool = True):
        if self.ifpose:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        frame = pe.process(frame, draw_pose=True, draw_face_mesh=True,
                           draw_face_detection=True, draw_boundingbox=True)
